...
export function WeatherConditions() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const getWindDirection = (degrees: number): string => {
        ...
        return windDirections[dirs[index]]
      }
  ...
  const isRain = weather?.condition.toLowerCase().includes("chuva")
  const isCloudy = weather?.condition.toLowerCase().includes("nublado")
  ...
  return (
    <section id="condicoes" className="relative py-24 bg-gradient-to-b from-cyan-50 via-sky-50 to-blue-100 overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-sky-200/40 to-transparent" />
      <div className="pointer-events-none absolute -bottom-10 left-0 right-0 h-24 opacity-50">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(14,165,233,0.35)_0%,_transparent_70%)] animate-pulse" />
      </div>
      <svg className="pointer-events-none absolute -bottom-1 left-0 w-[200%] h-24 text-cyan-300/40 animate-wave-drift" viewBox="0 0 1440 180" preserveAspectRatio="none" fill="currentColor">
      ...
      <div className="container mx-auto px-4">
        ...
        {surfCondition && (
          <div className="max-w-2xl mx-auto mb-8">
            <Card className="bg-gradient-to-br from-cyan-100 to-sky-100 border-cyan-300 shadow-lg">
